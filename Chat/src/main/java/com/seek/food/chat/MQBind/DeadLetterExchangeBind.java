...
public class DeadLetterExchangeBind {
    ...
    //绑定交换机与队列
    @Bean
    public Binding deleteFileChatImplBinding(Queue deleteFileChatImplQueue, DirectExchange deadLetterExchange){
        return BindingBuilder.bind(deleteFileChatImplQueue).to(deadLetterExchange).with(deadLetterExchangeConfig.getDeleteFileChatImplQueue().getRoutingKey());
    }
}
