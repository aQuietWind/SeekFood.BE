...
CREATE TABLE `first_comment` (
                                 ...
                                 `meal_name` bigint NOT NULL COMMENT '订单所点餐品的名字',
                                 `meal_content` bigint NOT NULL COMMENT '订单所点餐品的内容',
                                 ...
                                 `like_amount` int COMMENT '点赞数',
                                 `second_comment_amount` int COMMENT '二级评论数（子评论数）',
...
CREATE TABLE `second_comment` (
                                  ...
                                  `user_id` bigint COMMENT '发布该评论的用户id',
                                  `comment_image_addr` varchar(50) UNIQUE COMMENT '评论展示图片的地址',
                                  `comment_description` varchar(300) COMMENT '评论内容',
                                  `create_time` datetime NOT NULL default now() COMMENT '创建时间',
                                  `like_amount` int COMMENT '点赞数',
